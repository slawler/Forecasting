...
def FixGridAndValues(infile,xll,yll,ncols,nrows):
    ...
    xmrg_hourly.to_csv('%sfmt' %(infile), sep = ' ', float_format ='%.6f',
                       header = False, index = False, mode = 'a')
 
 
def MakeHourly(infile):
    import os, shutil,datetime
    dtm = datetime.datetime.strptime(infile[6:16],'%Y%m%d%H')
    z_offset = 0
    f_hour = int(infile[17:20])+ z_offset 
    d = datetime.timedelta(hours=f_hour)
    forecast = dtm + d
    newfile = forecast.strftime('%Y%m%d%H')
    basefile = str(newfile) + '.tmp'
    os.rename(infile,basefile)
    for i in range(6):
        hour = datetime.timedelta(hours=int(i))
        #print hour
        forecast_grid = forecast + hour
        FinalGrid = forecast_grid.strftime('%Y%m%d%H')
        shutil.copyfile(basefile,FinalGrid + '.asc') 
